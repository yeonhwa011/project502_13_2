...
public class JoinController extends AbstractController {
    ...
    public void prompt() {
        ...
        String confirmPw = promptWithValidation("비밀번호 확인: ",
                s->{boolean match = s.equals(userPw);
            if(!match) {
                System.err.println("\n비밀번호가 일치하지 않습니다.");
            }
                return match;
        ...
        String userNm = promptWithValidation("회원명: ",
                s->!s.isBlank());

        RequestJoin from = RequestJoin.builder()
                                          .userId(userId)
                                          .userPw(userPw)
                                          .confirmPw(confirmPw)
                                          .userNm(userNm)
